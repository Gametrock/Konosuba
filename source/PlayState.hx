...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'chiisana boukensha' | 'fantastic dreamer' | 'tomorrow':
		...
		switch(SONG.song.toLowerCase())
		{
			...
			case 'chiisana boukensha' | 'fantastic dreamer' | 'tomorrow' | 'tomorrow full': 
			{
				// defaultCamZoom = 0.9;
				curStage = 'house';

				var bg:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('konosuba/stagefiles/Fondo'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.1, 0.1);
				add(bg);

				var wall:FlxSprite = new FlxSprite(-60, 60).loadGraphic(Paths.image('konosuba/stagefiles/ParedDia'));
				wall.antialiasing = true;
				wall.scrollFactor.set(0.65, 0.9);
				wall.setGraphicSize(Math.floor(wall.width * 1.3));
				add(wall);

				var floor:FlxSprite = new FlxSprite(-60, 60).loadGraphic(Paths.image('konosuba/stagefiles/Piso'));
				floor.antialiasing = true;
				floor.scrollFactor.set(0.65, 0.9);
				floor.setGraphicSize(Math.floor(floor.width * 1.3));
				add(floor);

				var alfombra:FlxSprite = new FlxSprite(-150, 100).loadGraphic(Paths.image('konosuba/stagefiles/Alfombra'));
				alfombra.antialiasing = true;
				alfombra.scrollFactor.set(0.9, 0.9);
				alfombra.setGraphicSize(Math.floor(alfombra.width * 1.3));
				add(alfombra);

				var muebles:FlxSprite = new FlxSprite(-95, 50).loadGraphic(Paths.image('konosuba/stagefiles/Muebles'));
				muebles.antialiasing = true;
				muebles.scrollFactor.set(0.9, 0.9);
				muebles.setGraphicSize(Math.floor(muebles.width * 1.3));
				add(muebles);
			}
			default:
			{
		...
		if (PlayState.SONG.song.toLowerCase()=='chiisana boukensha' || PlayState.SONG.song.toLowerCase()== 'fantastic dreamer' || PlayState.SONG.song.toLowerCase()=='tomorrow') {
			dad.alpha = 0;
		} 
		...
		// GOTTA SHIT OFFSET
		if (SONG.player1.toLowerCase() == 'meguminplayable') {
			boyfriend.y -= 350;
		}
	...
	function startCountdown():Void
	{
		...
		startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
		{
			...
			{
				if (value == curStage && (curStage == "school" || curStage == "schoolEvil") )
				{
					introAlts = introAssets.get(value);
					altSuffix = '-pixel';
				}
				else if(value == curStage && curStage == "house") {
					introAlts = introAssets.get(value);
					altSuffix = '-ko';
				}
			}
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				var daDamage:Bool = songNotes[3] || false;
				...
				var swagNote:Note = new Note(daStrumTime, daNoteData, oldNote, false, daDamage);
				...
				if (storyPlaylist.length <= 0)
				{
					switch(curSong.toLowerCase()) {
						case 'tomorrow': 
							...
							FlxG.switchState(new Endings(accuracy));
	...
	private function popUpScore(daNote:Note):Void
		{ // Dont bully me bc I put the damage note thing here ;-;
			...
			var score:Float = 350;
			var damageNote = daNote.damageNote;
			...
			if (!damageNote) {
				...
				{
					case 'shit' | 'bad' | 'good':*/
						//trace('nooooooooob, atleast you arent like that one who made a sick smh');
						score = -300;
						combo = 0;
						misses++;
						health *= 0.6;
						...
						if (FlxG.save.data.accuracyMod == 0)
							totalNotesHit -= 0.50;
					/*case 'sick':
						trace('what a noob smh, not like i triggered this many times');
						ss = false;
						score = -10000;
						totalNotesHit -= 100;
						misses++;
			...
			if ((daRating != 'shit' || daRating != 'bad') && !damageNote)
				{
